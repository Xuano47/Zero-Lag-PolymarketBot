...
这是一个完全由 Rust 编写的、专为 Polymarket 打造的高性能、超低延迟套利抢单机器人。

它是对原 Go 版本引擎的彻底重写，旨在完全消除垃圾回收 (GC) 停顿，并在执行延迟上实现绝对的可预测性。在毫秒必争的套利竞争中，Rust 的内存管理机制保证了市场 WebSocket 流的处理不会出现像 GC 语言那样突然的延迟抖动。
